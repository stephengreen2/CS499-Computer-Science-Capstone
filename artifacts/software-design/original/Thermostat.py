...
class TemperatureMachine(StateMachine):
    ...
    def processTempIncButton(self):
        ...
        ## Increase the set point and update the lights
        self.setPoint += 1
        self.updateLights()
    ...
    def processTempDecButton(self):
        ...
        ## Decrease the set point and update the lights
        self.setPoint -= 1
        self.updateLights()
    ...
    def updateLights(self):
        ...
        if self.current_state.id == 'heat':
            if temp < self.setPoint:
                redLight.pulse() # fade red LED
            else:
                redLight.on() # solid red LED
        elif self.current_state.id == 'cool':
            if temp > self.setPoint:
                blueLight.pulse() # fade blue LED
            else:
                blueLight.on() # solid blue LED
        else:
            # 'off' state — all LEDs remain off
            redLight.off()
            blueLight.off()
    ...
    def setupSerialOutput(self):
        ...
        output = f"{self.current_state},{round(self.getFahrenheit())},{self.setPoint}"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ...
                lcd_line_2 = f"\nTemp: {round(self.getFahrenheit())}F"
    
                altCounter = altCounter + 1
            else:
                # display the current state and set point
                lcd_line_2 = f"\n{self.current_state} @ {self.setPoint}F"
            ...
            if((counter % 30) == 0):
                # send current state information to the server
...
# cycle thermostate when green button is pressed
...
##
redButton = Button(25)
# increase set point by one degree when red button is pressed
...
# decrease the set point by one degree when blue button is pressed
